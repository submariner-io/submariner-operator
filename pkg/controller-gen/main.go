...
func init() {
	for genName, gen := range allGenerators {
		...
		if helpGiver, hasHelp := gen.(genall.HasHelp); hasHelp {
			if hlp := helpGiver.Help(); hlp != nil {
				optionsRegistry.AddHelp(defn, hlp)
			}
		}
		...
		for ruleName, rule := range allOutputRules {
			...
			if helpGiver, hasHelp := rule.(genall.HasHelp); hasHelp {
				if hlp := helpGiver.Help(); hlp != nil {
					optionsRegistry.AddHelp(ruleMarker, hlp)
				}
			}
		}
	}
...
`,
	...
	cmd.Flags().CountVarP(&whichLevel, "which-markers", "w",
		"print out all markers available with the requested generators\n(up to -www for the most detailed output, or -wwww for json output)")
	cmd.Flags().CountVarP(&helpLevel, "detailed-help", "h",
		"print out more detailed help\n(up to -hhh for the most detailed output, or -hhhh for json output)")
