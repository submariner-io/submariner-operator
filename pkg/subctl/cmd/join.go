...
func joinSubmarinerCluster(config *rest.Config, clusterID string, subctlData *datafile.SubctlData) {
	...
	appliedServiceCIDR, err := getServiceCIDR(serviceCIDR, networkDetails)
	panicOnError(err)

	appliedClusterCIDR, err := getPodCIDR(clusterCIDR, networkDetails)
	panicOnError(err)

	fmt.Printf("* Deploying Submariner\n")
	err = deploy.Ensure(config, SubmarinerNamespace, repository, version,
		clusterID, appliedServiceCIDR, appliedClusterCIDR, colorCodes, nattPort,
