...
var _ = Describe("Broker webhook", func() {
	...
	BeforeEach(func(ctx context.Context) {
		...
		Expect(mcsv1b1.Install(scheme)).To(Succeed())
		...
		Entry("", "EndpointSlice", func() client.Object {
			return &discoveryv1.EndpointSlice{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "forbidden-eps",
					Namespace: brokerNamespace,
					Labels:    map[string]string{mcsv1b1.LabelSourceCluster: otherClusterID},
				},
			...
			return &mcsv1b1.ServiceImport{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						...
						mcsv1b1.LabelSourceCluster: otherClusterID,
					},
				},
	...
	Context("for aggregate ServiceImports", func() {
		...
		It("should only allow access to participating clusters", func(ctx context.Context) {
			...
			resourceClient := resource.ForControllerClient(otherClusterClient, brokerNamespace, &mcsv1b1.ServiceImport{})
			...
			Eventually(func(g Gomega) {
				g.Expect(util.MustUpdate(ctx, resourceClient, serviceImport, func(si *mcsv1b1.ServiceImport) (*mcsv1b1.ServiceImport, error) {
					si.Status.Clusters = append(si.Status.Clusters, mcsv1b1.ClusterStatus{Cluster: otherClusterID})
					return si, nil
				})).To(Succeed())
			}).Within(5 * time.Second).To(Succeed())
			...
			Eventually(func(g Gomega) {
				expectForbidden(g, util.MustUpdate(ctx, resourceClient, serviceImport, func(si *mcsv1b1.ServiceImport) (*mcsv1b1.ServiceImport, error) {
					si.Status.Clusters = slices.DeleteFunc(si.Status.Clusters, func(c mcsv1b1.ClusterStatus) bool {
