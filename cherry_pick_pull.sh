...
# meta.) Assumes you care about pulls from remote "my-fork" and
...
FORK_REMOTE=${FORK_REMOTE:-my-fork}
...
if [[ "$#" -lt 2 ]]; then
  ...
  echo
  echo "  Set UPSTREAM_REMOTE (default: origin) and FORK_REMOTE (default: my-fork)"
  echo "  to override the default remote names to what you have locally."
  echo
  echo "  For merge process info, see https://submariner.io/development/code-review/"
  exit 2
fi
...
if ! git log -n1 --format=%H "${BRANCH}" >/dev/null 2>&1; then
  echo "!!! '${BRANCH}' not found. The second argument should be something like ${UPSTREAM_REMOTE}/release-0.9."
...
EOF
  ...
  gh pr create --title="Automated cherry pick of ${numandtitle}" --body="${prtext}" --head "${GITHUB_USER}:${NEWBRANCH}" --base "${rel}" --repo="${MAIN_REPO_ORG}/${MAIN_REPO_NAME}" --label "backport"
