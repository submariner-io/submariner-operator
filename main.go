...
func main() {
	...
	setupLog.Info("Creating the engine CRDs")
	if err := engine.Ensure(crdUpdater); err != nil {
		setupLog.Error(err, "")
		os.Exit(1)
	}
	setupLog.Info("Creating the Lighthouse CRDs")
	...
	if err = (&submariner.SubmarinerReconciler{
	...
	if err = (&servicediscovery.ServiceDiscoveryReconciler{
		Client: mgr.GetClient(),
